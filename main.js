...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function render(a){
    ...
    out.innerHTML=`<p class="analysis-kind">${esc(a.analysis_type||'REAL DOCUMENT ANALYSIS')}</p><div class="metrics"><div><small>PAGES</small><strong>${a.page_count}</strong></div><div><small>OBJECTS ANALYZED</small><strong>${a.objects_analyzed}</strong></div><div><small>SUSPICIOUS OBJECTS</small><strong>${a.suspicious_objects}</strong></div><div class="risk ${riskClass}"><small>RISK SCORE</small><strong>${a.risk_score}<i>/100 · ${a.risk_level}</i></strong></div></div>
      ...
      <div class="result-actions"><button id="sanitizePdfBtn">GENERATE SANITIZED PDF</button><button id="sanitizeBtn">GENERATE SANITIZED TEXT</button><button id="jsonBtn">DOWNLOAD FINDINGS JSON</button><button id="reportBtn">DOWNLOAD AUDIT REPORT</button></div>
    ...
    $('sanitizeBtn').onclick=async()=>{try{const r=await fetch(apiUrl('/api/sanitize'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({analysis:a})});const s=await r.json();$('sanitized').innerHTML=`<h2>SANITIZED SEMANTIC REPRESENTATION</h2><p class="warning">${s.label} · The original PDF has not been rewritten.</p><h3>OBSERVED TEXT</h3><pre>${esc(s.observed_text.map(x=>`[Page ${x.page}] ${x.text}`).join('\n'))}</pre><h3>RETAINED TEXT</h3><pre>${esc(s.sanitized_text)||'(no retained text)'}</pre><h3>QUARANTINED TEXT · SPANS THAT TRIGGERED RULES</h3><pre class="quarantine">${esc(s.quarantined_text.map(x=>`[Page ${x.page}] ${x.text}`).join('\n'))||'None'}</pre><button id="textBtn">DOWNLOAD SANITIZED TEXT</button>`;$('textBtn').onclick=()=>download(s.sanitized_text,`${safe(a.filename)}-sanitized.txt`,'text/plain');requestAnimationFrame(()=>{$('sanitized').scrollIntoView({behavior:'smooth',block:'start'});});}catch{status('Sanitization request failed.','error');}};
    $('sanitizePdfBtn').onclick=async()=>{
      if(!selected)return;
      status('PHYSICAL SANITIZATION IN PROGRESS · REDACTING DETECTED SPANS','working');
      const fd=new FormData();fd.append('file',selected);
      try{
        const r=await fetch(apiUrl('/api/sanitize-pdf'),{method:'POST',body:fd});
        if(!r.ok){
          let detail='PDF sanitization request failed.';
          try{const errJson=await r.json();if(errJson.detail)detail=errJson.detail;}catch{}
          throw Error(detail);
        }
        const sanitizedObjects=r.headers.get('X-Sanitized-Objects')??String(a.suspicious_objects);
        const mode=r.headers.get('X-Sanitization-Mode')??'PHYSICAL-PDF';
        const blob=await r.blob();
        const originalName=selected?.name||a.filename||'document.pdf';
        const originalStem=originalName.replace(/\.pdf$/i,'');
        const sanitizedFilename=`${originalStem}-sanitized.pdf`;
        downloadBlob(blob,sanitizedFilename);
        status(`PHYSICAL SANITIZATION COMPLETE · ${sanitizedObjects} SUSPICIOUS OBJECTS REMOVED`,'done');
        $('sanitizedPdf').innerHTML=`<section class="sanitized-physical-card">
          <h2>PHYSICAL PDF SANITIZATION</h2>
          <div class="sanitization-details">
            <p><strong>Original PDF:</strong> ${esc(originalName)}</p>
            <p><strong>Suspicious objects detected:</strong> ${esc(a.suspicious_objects)}</p>
            <p><strong>Objects physically removed:</strong> ${esc(sanitizedObjects)}</p>
            <p><strong>Output:</strong> ${esc(sanitizedFilename)}</p>
            <p><strong>Mode:</strong> ${esc(mode)}</p>
          </div>
          <p class="warning">The original uploaded PDF is unchanged.</p>
          <button id="downloadPdfAgainBtn">DOWNLOAD SANITIZED PDF AGAIN</button>
        </section>`;
        $('downloadPdfAgainBtn').onclick=()=>downloadBlob(blob,sanitizedFilename);
      }catch(err){
        status(`Physical sanitization failed: ${err.message}`,'error');
      }
    };
  }
...
function download(content,name,type){downloadBlob(new Blob([content],{type}),name);}
