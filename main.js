...
document.addEventListener('DOMContentLoaded', () => {
  if (location.pathname.replace(/\/$/, '') === '/experiments') {
    import('./src/experiments/main.jsx').then(({ mountExperiments }) => mountExperiments(document.querySelector('.app-layout'), API)).catch(error => {
  ...
  function choose(file){
    ...
    if(file.size>20*1024*1024) return status('PDF exceeds the 20 MB upload limit.','error');
  ...
  async function runScan(){
    ...
    try{const response=await fetch(`${API}/api/analyze`,{method:'POST',body});const data=await response.json();if(!response.ok)throw Error(data.detail||'Analysis failed');latest=data;render(data);status(`${data.status} · ${data.objects_analyzed} actual text spans analyzed`,'done');}
    catch(err){status(`${err.message}. Confirm the FastAPI backend is running at ${API}.`,'error');}
